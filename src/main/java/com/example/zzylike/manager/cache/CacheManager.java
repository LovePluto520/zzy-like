package com.example.zzylike.manager.cache;
...
public class CacheManager {
    ...
    public TopK getHotKeyDetector() {
        hotKetDetector = new HeavyKeeper(
                //监控Top 100 Key
                100,
                //宽度
                100000,
    ...
    public Object get(String hashKey, String key) {
        ...
        if (value!=null){
            log.info("本地缓存获取到数据{}={}",compositeKey,key);
            //记录访问次数（每次访问计数+1）
            hotKetDetector.add(key,1);
            return value;
        }
        ...
        //3.记录访问（计数+1）
        AddResult addResult=hotKetDetector.add(key,1);

        //4.如果是热key且不在本地缓存，则缓存数据
